...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次裁剪为正方形
    powershell -Command ^
        ...
        "if (Test-Path \"$croppedFile\") { echo 裁剪后的文件已存在，跳过： \"$croppedFile\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在裁剪文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
