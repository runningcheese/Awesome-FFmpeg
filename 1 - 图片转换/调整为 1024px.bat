...
:process_folders
    ...
    powershell -Command ^
        ...
        "$outputFile = [System.IO.Path]::Combine($file.DirectoryName, [System.IO.Path]::GetFileNameWithoutExtension($file.Name) + '_1024px' + $file.Extension);" ^
        "if (Test-Path \"$outputFile\") { echo 文件已存在，跳过： \"$outputFile\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在调整文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-vf', 'scale=1024:1024:force_original_aspect_ratio=decrease', \"`\"$outputFile`\"\" -Wait; } }"
