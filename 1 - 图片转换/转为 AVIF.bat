...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为 AVIF 格式
    powershell -Command ^
        "$files = Get-ChildItem -Path '%folder%' -File -Recurse -Include '*.bmp','*.png','*.jpg', '*.jpeg', '*.gif', '*.tiff', '*.tif', '*.webp', '*.ico', '*.heic','*.heif', '*.avif', '*.svg';" ^
        "foreach ($file in $files) {" ^
        "$avifFile = [System.IO.Path]::ChangeExtension($file.FullName, '.avif');" ^
        "if (Test-Path \"$avifFile\") { echo AVIF 文件已存在，跳过： \"$avifFile\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在转换文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", \"`\"$avifFile`\"\" -Wait; }}"
