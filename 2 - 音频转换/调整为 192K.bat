...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件并依次转换为 192kbps MP3 格式
    powershell -Command ^
        ...
        "$mp3File = [System.IO.Path]::Combine($file.DirectoryName, [System.IO.Path]::GetFileNameWithoutExtension($file.Name) + '_192kbps.mp3');" ^
        "if (Test-Path \"$mp3File\") { echo MP3 文件已存在，跳过： \"$mp3File\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在转换文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-b:a', '192k', \"`\"$mp3File`\"\" -Wait; }}"
