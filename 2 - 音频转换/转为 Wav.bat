...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件并依次转换为 WAV 格式
    powershell -Command ^
        ...
        "$wavFile = [System.IO.Path]::ChangeExtension($file.FullName, '.wav');" ^
        "if (Test-Path \"$wavFile\") { echo WAV 文件已存在，跳过： \"$wavFile\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在转换文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-q:a', '0', \"`\"$wavFile`\"\" -Wait; }}"
