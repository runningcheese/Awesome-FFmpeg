...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为 epub 格式
    powershell -Command ^
        ...
        "if (Test-Path \"$epubFile\") { echo epub 文件已存在，跳过： \"$epubFile\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在转换文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "echo pandoc 命令: pandoc -ArgumentList \"$($file.FullName)\", '-o', \"$epubFile\";" ^
