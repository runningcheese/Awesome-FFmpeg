...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关音频文件，并依次截取音频片段为 MP3 格式
    powershell -Command ^
        "$files = Get-ChildItem -Path '%folder%' -File -Recurse -Include '*.mp3', '*.wav', '*.aac', '*.flac', '*.ogg', '*.wma', '*.ape';" ^
        "foreach ($file in $files) {" ^
        "$mp3File = [System.IO.Path]::ChangeExtension($file.FullName, '.mp3');" ^
        "$mp3File = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($mp3File), [System.IO.Path]::GetFileNameWithoutExtension($mp3File) + '_%start_time_filename%' + '.mp3');" ^
        "if (Test-Path \"$mp3File\") { echo MP3 文件已存在，跳过： \"$mp3File\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在截取文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-ss', '%start_time%', '-t', '%duration%', '-i', \"`\"$($file.FullName)`\"\", \"`\"$mp3File`\"\" -Wait; }}"
