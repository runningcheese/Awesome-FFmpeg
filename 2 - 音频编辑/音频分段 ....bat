...
REM 提示用户输入分割时长（分钟）
set /p part_duration=请输入每段的时长（分钟）： 

REM 转换分割时长为秒
set /a part_duration_seconds=part_duration * 60
...
REM 计算分割数量
set /a num_parts=total_seconds / part_duration_seconds

REM 分割操作
setlocal enabledelayedexpansion
for /l %%i in (0,1,%num_parts%) do (
    set /a start_time=%%i * part_duration_seconds

    if %%i equ %num_parts% (
        REM 处理最后一个不足设定时长的片段
        set /a last_duration=total_seconds-%%i*part_duration_seconds
        ...
        REM 处理每个设定时长的片段
        set /a file_num=%%i + 1
        ffmpeg -i "%input_file%" -ss !start_time! -t %part_duration_seconds% -q:a 0 "%output_folder%\%filename_without_ext%_!file_num!.mp3"
    )
)
