...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为 MOV 格式
    powershell -Command ^
        "$files = Get-ChildItem -Path '%folder%' -File -Recurse -Include '*.mp4', '*.mkv', '*.ts', '*.wmv', '*.avi', '*.mpg', '*.mpeg', '*.mov', '*.flv', '*.m4v', '*.rmvb', '*.3gp';" ^
        "foreach ($file in $files) {" ^
        "$movFile = [System.IO.Path]::ChangeExtension($file.FullName, '.mov');" ^
        "if (Test-Path \"$movFile\") { echo MOV 文件已存在，跳过： \"$movFile\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在转换文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "if ('%ffmpeg_hardware%' -ne '') {" ^
            "Start-Process -NoNewWindow -Wait ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '%ffmpeg_hardware%', '-c:a', 'aac', '-b:a', '192k', '-c:v', 'h264', '-preset', 'medium', '-crf', '23', \"`\"$movFile`\"\";" ^
        "} else {" ^
            "Start-Process -NoNewWindow -Wait ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-c:a', 'aac', '-b:a', '192k', '-c:v', 'h264', '-preset', 'medium', '-crf', '23', \"`\"$movFile`\"\";" ^
        "}}}"
