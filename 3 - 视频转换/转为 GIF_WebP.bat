...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为 WebP 动图格式
    powershell -Command ^
        ...
        "$webpFile = $webpFile.Replace('.webp', '_Origin.webp');" ^
        "if (Test-Path \"$webpFile\") { echo WebP 文件已存在，跳过： \"$webpFile\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在转换文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "$ffmpegArgs = @('-i', \"`\"$($file.FullName)`\"\", '-r', '10', '-vcodec', 'libwebp', '-lossless', '0', '-q:v', '80', '-loop', '0', \"`\"$webpFile`\"\");" ^
        "if ('%ffmpeg_hardware%' -ne '') { $ffmpegArgs = $ffmpegArgs.Insert(2, '%ffmpeg_hardware%'); }" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList $ffmpegArgs -Wait; }}" 
