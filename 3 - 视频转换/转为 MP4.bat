...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为 MP4 格式
    powershell -Command ^
        ...
        "if ('%ffmpeg_hardware%' -ne '') {" ^
            "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '%ffmpeg_hardware%', '-c:a', 'copy', \"`\"$mp4File`\"\" -Wait;" ^
        "} else {" ^
            "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-c:v', 'copy', '-c:a', 'copy', \"`\"$mp4File`\"\" -Wait;" ^
        "}}}"
