...
for %%F in (%*) do (
    ...
    ffmpeg.exe -i %%F -vf "crop=w=%firstImageWidth%:h=%firstImageHeight%,scale=%firstImageWidth%:%firstImageHeight%" "!outputFile!" 2>nul
    if errorlevel 1 (
...
echo 所有图片已裁剪为宽度：%firstImageWidth% 高度：%firstImageHeight% 并保存到文件夹：%folderName%
