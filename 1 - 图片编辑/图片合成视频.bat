...
for %%F in (%*) do (
    if /I "%%~xF"==".mp3" (
        ...
        if !index! equ 0 (
            ...
            set "outputFile=%%~dpF%%~nF_Video.mp4"
        )
        REM 将每个图片文件加入文件列表
        set "imageList=!imageList! %%F"
        REM 将图片文件调整为1920x1080分辨率
        ffmpeg.exe -i %%F -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,setdar=16/9" -q:v 1 "temp_!index!.jpg"
...
REM 去掉最后一个空格，并完成过滤器
set "concatFilter=%concatFilter:~0,-1% concat=n=!index!:v=1:a=0,format=yuv420p,setsar=1,setdar=16/9"
