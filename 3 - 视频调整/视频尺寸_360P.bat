...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为 360P MP4 格式
    powershell -Command ^
        ...
        "$newFileName = [System.IO.Path]::GetFileNameWithoutExtension($mp4File) + '_360P.mp4';" ^
        ...
        "echo 正在转换文件 \"$($file.FullName)\" 为 360P，请勿关闭窗口...;" ^
        "$args = @('-i', \"`\"$($file.FullName)`\"\", '-vf', 'scale=-1:360');" ^
