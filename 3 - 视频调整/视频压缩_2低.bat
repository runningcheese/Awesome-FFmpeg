...
:process_folders
    ...
    powershell -Command ^
        ...
        "$mp4File = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($mp4File), [System.IO.Path]::GetFileNameWithoutExtension($mp4File) + '_Low' + [System.IO.Path]::GetExtension($mp4File));" ^
        ...
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '$ffmpeg_hardware', '-crf', '20', '-maxrate', '1000k', '-bufsize', '2000k', '-c:a', 'aac', '-b:a', '128k', \"`\"$mp4File`\"\" -Wait;" ^
        "} else {" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-c:v', 'libx264', '-crf', '20', '-maxrate', '1000k', '-bufsize', '2000k', '-c:a', 'aac', '-b:a', '128k', \"`\"$mp4File`\"\" -Wait;" ^
        "}}}"
