...
:process_folders
    ...
    powershell -Command ^
        ...
        "$mp4File = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($mp4File), [System.IO.Path]::GetFileNameWithoutExtension($mp4File) + '_High' + [System.IO.Path]::GetExtension($mp4File));" ^
        ...
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '$ffmpeg_hardware', '-crf', '18', '-maxrate', '4000k', '-bufsize', '8000k', '-c:a', 'aac', '-b:a', '192k', \"`\"$mp4File`\"\" -Wait;" ^
        "} else {" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-c:v', 'libx264', '-crf', '18', '-preset', 'slow', '-maxrate', '4000k', '-bufsize', '8000k', '-c:a', 'aac', '-b:a', '192k', \"`\"$mp4File`\"\" -Wait;" ^
        "}}}"
