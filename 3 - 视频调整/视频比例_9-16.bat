...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为 9:16 竖屏比例的 MP4 格式
    powershell -Command ^
        ...
        "$mp4File = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($mp4File), [System.IO.Path]::GetFileNameWithoutExtension($mp4File) + '_9-16' + [System.IO.Path]::GetExtension($mp4File));" ^
        ...
        "$targetWidth = [int]($height * 9 / 16);" ^
        "$targetHeight = [int]($width * 16 / 9);" ^
