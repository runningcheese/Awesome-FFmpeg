...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为裁剪为 4:3 比例的 MP4 格式
    powershell -Command ^
        ...
        "$mp4File = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($mp4File), [System.IO.Path]::GetFileNameWithoutExtension($mp4File) + '_4-3' + [System.IO.Path]::GetExtension($mp4File));" ^
        ...
        "$newWidth = $width;" ^
        "$newHeight = [int]($width * 3 / 4);" ^
        "if ($newHeight -gt $height) {" ^
        "$newHeight = $height;" ^
        "$newWidth = [int]($height * 4 / 3);" ^
        "}" ^
        "$xOffset = [int](($width - $newWidth) / 2);" ^
        "$yOffset = [int](($height - $newHeight) / 2);" ^
        "$cropFilter = \"crop=${newWidth}:${newHeight}:${xOffset}:${yOffset}\";" ^
        ...
        "    Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-vf', $cropFilter, $env:ffmpeg_hardware, \"`\"$mp4File`\"\" -Wait;" ^
        "} else {" ^
        "    Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-vf', $cropFilter, \"`\"$mp4File`\"\" -Wait;" ^
        "} }}"
