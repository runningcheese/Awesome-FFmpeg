...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为压缩的 MP4 格式
    powershell -Command ^
        ...
        "$mp4File = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($mp4File), [System.IO.Path]::GetFileNameWithoutExtension($mp4File) + '_Ultrahigh' + [System.IO.Path]::GetExtension($mp4File));" ^
        "if (Test-Path \"$mp4File\") { echo MP4 文件已存在，跳过： \"$mp4File\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在压缩文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "if ('%ffmpeg_hardware%' -ne '') {" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '$ffmpeg_hardware', '-crf', '16', '-maxrate', '6000k', '-bufsize', '12000k', '-c:a', 'aac', '-b:a', '192k', \"`\"$mp4File`\"\" -Wait;" ^
        "} else {" ^
        "Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-c:v', 'libx264', '-crf', '16', '-preset', 'slow', '-maxrate', '6000k', '-bufsize', '12000k', '-c:a', 'aac', '-b:a', '192k', \"`\"$mp4File`\"\" -Wait;" ^
        "}}}"
