...
:process_folders
    ...
    "$gifFile = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($gifFile), [System.IO.Path]::GetFileNameWithoutExtension($gifFile) + '_%start_time_filename%' + '.gif');" ^
    "if (Test-Path \"$gifFile\") { echo GIF 文件已存在，跳过： \"$gifFile\"; Start-Sleep -Seconds 1; } else {" ^
    "echo 正在截取文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
    "Start-Process -NoNewWindow ffmpeg -ArgumentList '-ss', '%start_time%', '-t', '%duration%', '-i', \"`\"$($file.FullName)`\"\", '-vf', 'scale=640:-1:flags=lanczos', '-r', '10', \"`\"$gifFile`\"\" -Wait; }}"
