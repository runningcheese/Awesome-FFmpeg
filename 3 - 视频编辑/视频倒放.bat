...
REM 设置编码器参数和比特率
SET "ffmpeg_codec=-c:v libx264 -b:v 5M -preset veryfast"
...
:process_folders
    ...
    REM 使用 PowerShell 获取指定文件夹内的所有相关文件，并依次转换为倒放的 MP4 格式
    powershell -Command ^
        ...
        "$mp4File = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($mp4File), [System.IO.Path]::GetFileNameWithoutExtension($mp4File) + '_Reversed' + [System.IO.Path]::GetExtension($mp4File));" ^
        "if (Test-Path \"$mp4File\") { echo MP4 文件已存在，跳过： \"$mp4File\"; Start-Sleep -Seconds 1; } else {" ^
        "echo 正在倒放文件 \"$($file.FullName)\" 请勿关闭窗口...;" ^
        "if ('%ffmpeg_hardware%' -ne '') {" ^
        "  Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-vf', 'reverse', '-af', 'areverse', '%ffmpeg_hardware%', '%ffmpeg_codec%', \"`\"$mp4File`\"\" -Wait;" ^
        "} else {" ^
        "  Start-Process -NoNewWindow ffmpeg -ArgumentList '-i', \"`\"$($file.FullName)`\"\", '-vf', 'reverse', '-af', 'areverse', '%ffmpeg_codec%', \"`\"$mp4File`\"\" -Wait;" ^
        "}}}"
