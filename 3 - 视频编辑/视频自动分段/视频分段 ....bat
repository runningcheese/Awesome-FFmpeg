...
REM 获取用户输入的分割时长（分钟）
set /p "user_input_duration=请输入分割时长（单位：分钟）："

REM 将用户输入的分钟数转换为秒数
set /a part_duration=user_input_duration * 60
...
for /l %%i in (0,1,%num_parts%) do (
    ...
    if %%i equ %num_parts% (
        ...
        if !last_duration! gtr 0 (
            set /a file_num=%%i + 1
            ffmpeg -i "%input_file%" -ss !start_time! -t !last_duration! -c copy "%output_folder%\%filename_without_ext%_!file_num!.mp4"
        )
    ) else (
        REM 处理每个设定时长的片段
        set /a file_num=%%i + 1
        ffmpeg -i "%input_file%" -ss !start_time! -t %part_duration% -c copy "%output_folder%\%filename_without_ext%_!file_num!.mp4"
    )
)
